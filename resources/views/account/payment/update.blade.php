...
@section('content')
    <div class="card-body">
        ...
        <form action="/account/payment/{{ $payment->CardNumber }}" method="POST" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="form-group">
                <label for="CardNumber">Card Number</label>
                <input type="string" class="form-control" id="CardNumber" placeholder="Example: 1234 5678 9101 1123"
                    name="CardNumber" value="{{ old('CardNumber', $payment->CardNumber) }}">
                @error('CardNumber')
                    {{ $message }}
                @enderror
            </div>
            <div class="row">
                <div class="col">
                    ...
                    <input type="date" class="form-control" id="ExpiredDate" placeholder="" name="ExpiredDate"
                        value="{{ old('ExpiredDate', $payment->ExpiredDate) }}">
                    @error('ExpiredDate')
                        {{ $message }}
                    @enderror
                </div>
